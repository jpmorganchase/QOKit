...
def fine_tune_result(f_overlap, f, gs_energy, N, p, num_coeffs, gamma, beta, basis, optimizer='bobyqa', rhobeg=None, maxiter=1000, max_energy=0):
    res, res_gamma, res_beta = fine_tune_coeffs(f_overlap, f, gs_energy, N, p, num_coeffs, gamma, beta, basis, optimizer=optimizer, rhobeg=rhobeg, maxiter=maxiter, max_energy=max_energy)
    ...
    nfev = res['nfev']
    result = {'N':N, 'p':p, 'num_coeffs': num_coeffs, 'gamma': res_gamma, 'beta': res_beta, 'basis': basis, 'u': u, 'v': v,  'optimizer': optimizer, 'approx_ratio': approx_ratio, 
        'merit': merit, 'overlap': overlap, 'evaluations': nfev}
    return result 
