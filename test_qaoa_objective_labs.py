...
class TestBestParamsMatchValues:
    ...
    def test_best_params_match_values(self, objective, simulator):
        if objective == "expectation":
            ...
            def convert_result(x): return -x
        else:
            suffix_to_check = " opt4overlap"
            col_name = "overlap" + suffix_to_check
            def convert_result(x): return 1 - x
        ...
        for _, row in tqdm(
            ...
            f = get_qaoa_labs_objective(
                row["N"], row["p"], objective=objective, simulator=simulator)

            x = np.hstack((row["gamma" + suffix_to_check],
                          row["beta" + suffix_to_check]))
...
def test_return_both_objectives(simulator):
    ...
    e1 = get_qaoa_labs_objective(
        N, p, objective="expectation", simulator=simulator)(theta)
    o1 = get_qaoa_labs_objective(
        N, p, objective="overlap", simulator=simulator)(theta)
    e2, o2 = get_qaoa_labs_objective(
        N, p, objective="expectation and overlap", simulator=simulator)(theta)
...
def test_best_known_LABS_parameters(simulator):
    ...
    o = get_qaoa_labs_objective(
        N, p, objective="overlap", parameterization="gamma beta", simulator=simulator)(gamma, beta)
