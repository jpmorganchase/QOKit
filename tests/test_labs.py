...
def test_depth_opt_terms_generation():
    for N in range(3, 16):
        terms_with_weight, offset = get_terms_offset(N)
        terms_indices = [term[1] for term in terms_with_weight]
        assert set(terms_indices) == set(get_depth_optimized_terms(N))
...
def test_gate_optimized_terms_naive():
    for N in range(3, 16):
        terms_with_weight, offset = get_terms_offset(N)
        terms_indices = [term[1] for term in terms_with_weight]
        assert set(terms_indices) == set(get_gate_optimized_terms_naive(N, number_of_gate_zones=4))
        assert set(terms_indices) == set(get_gate_optimized_terms_naive(N, number_of_gate_zones=None))
...
def test_gate_optimized_terms_greedy():
    for N in range(3, 16):
        terms_with_weight, offset = get_terms_offset(N)
        terms_indices = [term[1] for term in terms_with_weight]
        assert set(terms_indices) == set(get_gate_optimized_terms_greedy(N, number_of_gate_zones=4))
        assert set(terms_indices) == set(get_gate_optimized_terms_greedy(N, number_of_gate_zones=None))
