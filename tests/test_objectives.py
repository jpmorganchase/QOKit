...
simulators_to_run = ("vectorized", "qiskit", "cfurx")
if numba.cuda.is_available():
    simulators_to_run = simulators_to_run + ("nbcufurx",)
else:
    warnings.warn("Skip GPU tests as no compatible devices are found.")


class TestBestParamsMatchValues(object):
    ...
    @pytest.mark.parametrize(
        "objective, simulator",
        list(itertools.product(("expectation", "overlap"), simulators_to_run)),
    )
...
def test_return_both_objectives():
    N = 7
    p = 4

    theta = np.random.uniform(0, 1, 2 * p)

    e1 = get_qaoa_labs_objective(N, p, objective="expectation")(theta)
    o1 = get_qaoa_labs_objective(N, p, objective="overlap")(theta)
    e2, o2 = get_qaoa_labs_objective(N, p, objective="expectation and overlap")(theta)
